...
describe User, type: :model do
  ...
  describe 'ユーザー新規登録'do
    context '新規登録ができない' do
      ...
      it 'passwordが存在してもencrypted_password_confirmationが空では登録できない' do
        @user.password = "j00000"
        @user.password_confirmation = ""
        @user.valid?
        expect(@user.errors.full_messages).to include("Password confirmation doesn't match Password")
      end
      it 'passwordが英数字ともに使用されていない場合登録できない' do
    ...
    context '新規登録ができるとき' do
      ...
      it 'passwordとencrypted_password_confirmationが6文字以上且つ、半角英数字を使用していれば登録できる' do
